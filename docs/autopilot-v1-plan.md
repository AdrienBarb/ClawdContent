...
> **AMENDMENT 2026-06-18 — rolling weeks supersede calendar weeks.** The "every Sunday 17:00 → Monday-to-Sunday calendar week" model below is replaced by a **per-user rolling 7-day window** anchored on each user's first-generation (checkout) day, advancing 7 days each cycle. The hourly `autopilot-dispatch` computes each user's next window via `computeDueAnchor` and fires `autopilot/generate-week` (`reason: "recurring"`) the **evening before** the window starts (`generationFireAt` = anchor−1day @ 17:00 local — so existing Monday-anchored users keep the same Sunday-evening timing). `planSlotDates` clamps every post to `[anchor, anchor+7d)`. Removed: the `coverFrom` stitching, `nextWeekStart`/`currentWeekStart`, the `first_week`-vs-`weekly` planning split, and the in-function digest `sleepUntil` (digest now sends right after finalize). `WeeklyBatch.weekStart` now stores the rolling anchor, not a calendar Monday — no schema/Prisma migration. Pure logic lives in `src/lib/services/autopilot/schedule.ts` (unit-tested in `schedule.test.ts` + `time.test.ts`). Read everything below as historical context; the dispatch/anchor mechanics are superseded by this note.
